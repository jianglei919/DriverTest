        ...
        if (driverInfo.TestResult === 'PENDING') {
            actionCell = `
                    <form class="updateForm" data-id="${driverInfo._id}">
                        <textarea name="comment" placeholder="Add comment" required>${driverInfo.Comment || ''}</textarea>
            ...
            success: function (result) {
                if (result.success) {
                    ...
                    $form.closest('tr').find('td:last-child').html(`
                        <span>Test Result: ${result.updatedDriver.TestResult}</span>
                        <p>Comment: ${result.updatedDriver.Comment || 'No comment'}</p>
                    `);
                } else {
