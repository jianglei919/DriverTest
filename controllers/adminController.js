...
module.exports = {
  ...
  addAppointment: async (req, res) => {
    try {
      ...
      if (existingAppointment) {
        return res.json({
          success: false,
          message: `Time slot already exists for date: ${date}, time: ${time}`,
        });
      }
      ...
      res
        .status(500)
        .json({ success: false, message: "Error adding time-slot." });
    }
  },
  ...
  getCandidates: async (req, res) => {
    try {
      ...
      const candidates = await DriverTestInfo.find({
        UserType: "Driver",
        LicenseNo: { $ne: "default" },
      });

      // 查询所有订单，避免重复创建订单
      const orders = await OrderInfo.find();
      const orderedCandidateIds = orders.map((order) =>
        order.candidateId.toString()
      );
    ...
    try {
      ...
      if (!candidate || candidate.TestResult !== "PASS") {
        req.flash("validationErrors", [
          "Invalid candidate or candidate not passed",
        ]);
      ...
      if (existingOrder) {
        req.flash("validationErrors", [
          "Order already exists for this candidate",
        ]);
        return res.redirect("/admin/candidate");
      }
      ...
      req.flash(
        "success",
        `Order created for ${candidate.firstname} ${candidate.lastname}`
      );
    } catch (error) {
